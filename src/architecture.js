...
export function buildArchitecture(app,geo,frontages){
 ...
 // Context walls carry a window rhythm so the cross-street views are not blank cardboard. One tile = 3.2m x 3.84m (uvscale .5).
 const contextWall=(hex,brickLike)=>{const key='ctx'+hex;if(colors[key])return colors[key];const m=new pc.StandardMaterial();m.name=key;m.gloss=.14;m.specular=new pc.Color(.12,.12,.12);const b=hex.match(/../g).map(v=>parseInt(v,16));
  m.diffuseMap=texture((c,w,h)=>{c.fillStyle='#'+hex;c.fillRect(0,0,w,h);if(brickLike)for(let y=0;y<24;y++)for(let x=-1;x<8;x++){const v=(rnd()-.5)*22;c.fillStyle=`rgb(${b[0]+v},${b[1]+v*.8},${b[2]+v*.7})`;c.fillRect(x*64+(y%2)*32+1,y*21+1,62,19);}for(let i=0;i<9000;i++){c.fillStyle=rnd()>.5?'#ffffff0a':'#0000000c';c.fillRect(rnd()*w,rnd()*h,2,1);}
   // one sash window per tile, sill band beneath
   c.fillStyle='#d8d3c4';c.fillRect(176,118,160,236);c.fillStyle='#233033';c.fillRect(190,132,132,208);c.fillStyle='#4b6068';c.fillRect(196,138,120,90);c.fillStyle='#d8d3c4';c.fillRect(252,132,8,208);c.fillRect(190,232,132,8);c.fillStyle='#00000022';c.fillRect(176,354,160,10);});
  m.cull=pc.CULLFACE_NONE;m.update();return colors[key]=m;};
 for(const f of geo.footprints){if(registered.has(f.id)||skip.has(f.id))continue;const b=f.bounds;if(Math.abs((b.minX+b.maxX)/2)>125||b.maxZ<-100||b.minZ>550)continue;const h=9+(f.id%4)*2,base=geo.groundAt((b.minZ+b.maxZ)/2),p=new pc.Entity('Context footprint '+f.id);p.setPosition(0,base,0);root.addChild(p);const m=contextWall(['ad7960','c5baa2','ab806a','a49e8a'][f.id%4],f.id%4!==1);for(const r of f.rings){for(let i=0;i<r.length-1;i++)sheet('Context wall',[[r[i][0],-.5,r[i][1]],[r[i+1][0],-.5,r[i+1][1]],[r[i+1][0],h,r[i+1][1]],[r[i][0],h,r[i][1]]],m,p,.5);polygonRoof(r,h,roof,p);}}
