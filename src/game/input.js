export function createInput(canvas, actions){
  ...
  // `lookScale` lets the game slow the reticle while it passes over a target.
  function look(dx,dy,mobile=false){const stick=actions.lookScale?.()??1;let gx=.13,gy=.11;if(mobile){const boost=1+.7*Math.min(1,Math.hypot(dx,dy)/16);gx=.40*boost;gy=.32*boost;}state.mx+=dx*gx*state.sensitivity*stick;state.my+=dy*gy*state.sensitivity*stick;}
  window.addEventListener('keydown',e=>{if(e.target instanceof HTMLInputElement)return;if(actions.runnerActive?.()){const direction={KeyA:'left',ArrowLeft:'left',KeyD:'right',ArrowRight:'right',KeyW:'up',ArrowUp:'up',Space:'up',KeyS:'down',ArrowDown:'down'}[e.code];if(direction){e.preventDefault();if(!e.repeat)actions.runnerAction(direction);return;}if(!['KeyP','Escape','KeyM'].includes(e.code))return;}if(['KeyW','KeyA','KeyS','KeyD','ArrowUp','ArrowDown','ArrowLeft','ArrowRight','Space'].includes(e.code))e.preventDefault();keys.add(e.code);if(e.repeat)return;if(e.code==='KeyQ')actions.yeetStart();if(e.code==='KeyG')actions.equipment('grenade');if(e.code==='KeyV')actions.equipment('barricade');if(e.code==='KeyT')actions.equipment('barrel');if(e.code==='KeyB')actions.skate();if(e.code==='KeyJ')actions.trick('kickflip');if(e.code==='KeyK')actions.trick('heelflip');if(e.code==='KeyL')actions.trick('shove');if(e.code==='KeyR')actions.reload();if(e.code==='Digit1')actions.weapon('pistol');if(e.code==='Digit2')actions.weapon('shotgun');if(e.code==='KeyE')actions.continue();if(e.code==='KeyP'||e.code==='Escape'&&actions.active())actions.pause();if(e.code==='KeyM')actions.mute();});
  ...
  let swipe=null;const swipeZone=$('#runner-swipe');swipeZone.addEventListener('pointerdown',e=>{if(!actions.runnerActive())return;e.preventDefault();swipe={id:e.pointerId,x:e.clientX,y:e.clientY};capture(swipeZone,e.pointerId);});
  swipeZone.addEventListener('pointerup',e=>{if(!swipe||swipe.id!==e.pointerId)return;const dx=e.clientX-swipe.x,dy=e.clientY-swipe.y;swipe=null;if(Math.max(Math.abs(dx),Math.abs(dy))<26)return;actions.runnerAction(Math.abs(dx)>Math.abs(dy)?dx>0?'right':'left':dy>0?'down':'up');});
  for(const ev of ['pointercancel','lostpointercapture'])swipeZone.addEventListener(ev,()=>swipe=null);
  return state;
}
