...
export function buildSkateCourse(app,world){
 ...
 for(const rail of data.rails){if(rail.bench)continue;const z=(rail.z0+rail.z1)/2,length=rail.z1-rail.z0;const top=box(rail.name+' grind rail',rail.x,rail.height,z,.11,.11,length,steel);top.setEulerAngles(-Math.atan2(world.terrainAt(rail.x,rail.z1)-world.terrainAt(rail.x,rail.z0),length)*180/Math.PI,0,0);
