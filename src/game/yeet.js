...
export function createYeet(hooks){
 ...
 function begin(){if(held||cooldown>0)return;const p=hooks.player();target=selectYeetTarget(hooks.enemies(),p,hooks.direction(),hooks.visible);if(!target){hooks.toast('YEET · Aim at a zombie or red barrel within 5 metres');return;}held=true;time=0;hooks.sound('ready');}
 function release(){if(!held)return;held=false;const e=target;target=null;const p=hooks.player(),power=chargeAt(time);if(!hooks.enemies().includes(e)||e.flight||Math.hypot(e.x-p.x,e.z-p.z)>YEET.range+1||!hooks.visible(p.x,p.z,e.x,e.z,e)){hooks.toast('YEET · Target out of reach');return;}
  ...
  const v=launchVelocity(hooks.direction(),power);e.flight=true;hooks.launched?.(e);flights.push({enemy:e,x:e.x,y:hooks.floor(e.x,e.z)+.3,z:e.z,vx:v.x,vy:v.y,vz:v.z,age:0,hits:new Set(),power});hooks.sound('yeet');hooks.toast(e.kind==='barrel'?'BARREL YEET! · Explodes on impact':'YEET! · Bowl through the horde');
 }
 function update(dt){cooldown=Math.max(0,cooldown-dt);if(held)time+=dt;
  for(let i=flights.length-1;i>=0;i--){const f=flights[i],e=f.enemy;if(!hooks.enemies().includes(e)){flights.splice(i,1);continue;}f.age+=dt;let impact=false;
   ...
   if(!hooks.enemies().includes(e)){flights.splice(i,1);continue;}hooks.pose(e,f);if(impact||f.age>4){e.flight=false;hooks.damage(e,1000);flights.splice(i,1);}
  }
 }
