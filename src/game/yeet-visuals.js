...
export function createYeetVisuals(app,world,hooks){
 ...
 function update(yeet,enabled){for(const d of dots)d.enabled=false;marker.enabled=false;if(!enabled||yeet.cooldown>0)return;
  const target=yeet.target||selectYeetTarget(hooks.enemies(),hooks.player(),hooks.direction(),hooks.visible);if(!target)return;marker.enabled=true;marker.setPosition(target.x,world.terrainAt(target.x,target.z)+2.15*(target.scale||1),target.z);marker.setEulerAngles(0,hooks.yaw(),0);
