...
test('Skate pushes, brakes and reverses at compact street speeds',()=>{const s=createSkater();s.active=true;tick(s,3,{...neutral,forward:1});assert(s.speed>=8&&s.speed<=10.3);assert(s.z<-15);tick(s,1,{...neutral,forward:-1});assert(s.speed<1);});
